...
explanation = These users do not belong according to the README.
...
explanation = These users must be added according to the README.
...
explanation = These are required users.
...
explanation = Frank must be an admin according to the README.
...
explanation = Dennis is not allowed to be an admin according to the README.
...
explanation = Users charlie and deandra should be added to the webdev group according to the README
...
explanation = The README asks for the webdev group to be created.
...
explanation = Lots of different ways to help secure the login.
...
explanation = Do the current users' passwords conform to your password policy? They should!
...
explanation = Lots of PAM parameters that can be enforced.
...
explanation = Password policy changes.
...
explanation = These pieces of software are unauthorized. John is a pw cracker. Tightvncserver allows remote access (only ssh is authorized). Netdiscover is used by badguys to quickly enumerate networks.
...
explanation = Firewall should always be enabled!
...
explanation = Many default settings are just fine for ssh. There are a few you should change to tighten up security.
...
explanation = These services must be running according to the README.
...
explanation = MySQL is not a critical service and should be disabled. It is dangerous to leave a mysql server exposed to the network.
...
explanation = Update settings should be changed to the most paranoid possible to ensure quick installation of patches.
...
explanation = There is SO MUCH to unpack on this one. I'll save it for later.
...
explanation = The /etc/passwd contains important account info that should not be writable by normal users.
...
explanation = /var/www is the root directory and must be accessible to the webdev group according to the README.
...
explanation = Plaintext passwords should NEVER be stored on a shared computer, especially one destined to be a webserver.
...
explanation = Ssh should display a banner to users to warn them about connecting to the service.
...
explanation = Frank is NOT special. Even he has to login with a pw.
...
explanation = All users must have passwords
...
explanation = Cronjob must be added to the root crontab
...
explanation = Apache might be serving a website, but it isn't available to the outside world without a firewall rule
...
explanation = Charlie is a wildcard and must not be allowed a normal bash shell.
...
explanation = Dennis is not to be logging in via ssh according to the README
...
explanation = /etc/passwd should only be owned by root. This would allow frank to do things like add access to accounts that shouldn't have it.
...
explanation = The group owner must be changed with chown to match the parameters set forth by the README file.
...
explanation = Backdoors allow attackers unauthorized access to the OS
